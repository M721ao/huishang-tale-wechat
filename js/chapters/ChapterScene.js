...
export class ChapterScene {
  ...
  drawChapterButton(button) {
    ...
    // 绘制按钮背景
    ctx.fillStyle = chapter.unlocked
      ? "rgba(0, 0, 0, 0.6)"
    ...
    ctx.fillText(chapter.title, button.x, button.y - 20 - this.scrollY);

    // 绘制章节描述
    ctx.font = "16px Arial";
    ctx.fillText(chapter.description, button.x, button.y + 10 - this.scrollY);
