...
export class TitleScene {
    ...
    handleTap(x, y) {
        ...
        if (guestText.visible && 
            x >= guestText.x - 50 && x <= guestText.x + 50 &&
            y >= guestText.y - 20 && y <= guestText.y + 20) {
            if (this.onGuestMode) {
                this.onGuestMode()
            }
        }
    ...
    drawButton(ctx, button, text, color = '#D4AF37') {
        ...
        ctx.shadowBlur = 2
        ctx.font = button.height > 45 ? this.uiHelper.getFont(28, 'FangSong', true) : this.uiHelper.getFont(22, 'FangSong', true)
